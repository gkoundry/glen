...
for col in ('A','B','C','D','E','F','G'):
    ...
    f=open('train4'+col+'ca.csv','w')
    f.write("id,y,ls,mf,agemf,prls,prmf,frls,frmf,quotes,nfrls,nfrmf,frrt,prrt,csls,csmf,risk0,risk1,risk2,risk3,risk4,csdir,G1,G2,G3,G4,ca\n")
    for id in ans.keys():
        ls = last[id][col]
        #mf = mfreq[id][col]
        mf = llast[id][col]
